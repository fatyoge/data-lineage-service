...
class Config implements IConfig {
  ...
  get iotaProviders(): string[] {
    if (!this._iotaProviders) {
      Logger.log(
        "Reading iotaProviders and internal field is undefined, trying to load from data file cache"
      );
      this._iotaProviders = this._fileData.getKey("iotaProviders");
      if (!this._iotaProviders) {
        Logger.log(
          "iotaProviders loaded from data file cache, but has no data, use an empty array to initialize it"
        );
        this._iotaProviders = [];
      } else {
        Logger.log(
          `iotaProviders loaded from data file cache with the data ${JSON.stringify(
            this._iotaProviders,
            null,
            4
          )}`
  ...
  set iotaProviders(values: string[]) {
    ...
    Logger.log(
      `iotaProviders is changed to ${JSON.stringify(
        this._iotaProviders,
        null,
        4
      )}`
  ...
  useProxyIfConfigured() {
    ...
    if (c.proxy) {
      Logger.log(
        "server-config is configured with a proxy, use it for all http(s) request from server"
      );
